...
export default function CreateSubscriptionPage() {
  ...
  const fetchSalons = async () => {
    try {
      ...
      const response = await fetch('/api/salons?status=active');
      if (!response.ok) throw new Error('Failed to fetch salons');
      
      const data = await response.json();
      // Filter out salons that already have active subscriptions
      const availableSalons = data.salons.filter((salon: any) => !salon.hasActiveSubscription);
      setSalons(availableSalons);
      setFilteredSalons(availableSalons);
    } catch (error) {
